    ...
    var body: some View {
        Form {
            ...
            Section("Community") {
                HStack(spacing: 16) {
                    Button {
                        openURL(URL(string: "https://github.com/DinanathDash/Stasis/issues/new?template=bug_report.yml")!)
                    } label: {
                    ...
                    Button {
                        openURL(URL(string: "https://github.com/DinanathDash/Stasis/issues/new?template=feature_request.yml")!)
                    } label: {
                    ...
                    Button {
                        openURL(URL(string: "https://github.com/DinanathDash/Stasis")!)
                    } label: {
                        Label {
                            Text("View on GitHub")
                        } icon: {
            ...
            Section {
                ...
                Picker("Check frequency", selection: $updateCheckInterval) {
                    Text("At start").tag(UpdaterService.UpdateCheckInterval.atStart)
                    Divider()
                    Text("Once per day").tag(UpdaterService.UpdateCheckInterval.daily)
                    Text("Once per week").tag(UpdaterService.UpdateCheckInterval.weekly)
                    Text("Once per month").tag(UpdaterService.UpdateCheckInterval.monthly)
                }
